    ...
    var SccSimpleupload = SimpleSplunkView.extend({
        ...
        render: function() {
            ...
            $uploaded_list.on('click', function(e){
                ...
                if($target.hasClass('icon-close')){
                    ...
                    if(confirm('Are you sure you want to delete '+ file_name)){
                        $.ajax({
                            url: '/custom/splunk_components_collection/scc_simpleupload_service/remove/'+ file_name,
                            data: { app: SplunkUtils.getCurrentApp() },
                        }).done(function(){
                            $file.remove();
                        }).error(function(){
            ...
            function getServerFiles(){
                $.ajax({
                    url: '/custom/splunk_components_collection/scc_simpleupload_service/list',
                    data: { app: SplunkUtils.getCurrentApp() },
                }).done(function(files){
